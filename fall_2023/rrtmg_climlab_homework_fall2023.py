...
relh = 0.8 # If idealized_moisture_option = 1, this sets the fractional relative humidity at the surface
		   # If idealized_moisture_option = 2, this sets a constant (fractional) relative humidity value
...
ozonepath = '/Users/scavallo/Documents/data/ozone/apeozone_cam3_5_54.nc'
...
temparr = np.array(temperature)
presarr = np.array(pressure)
dh2o_dp = np.zeros_like(temparr).astype('f')
dh2o_dp[:] = float('NaN')  
dh2o_dp[1::] = ( (temparr[1::]-temparr[0:-1]) / (presarr[1::]-presarr[0:-1]) ) * 100.
...
fig = pylab.figure(**figprops)   # New figure
ax1 = fig.add_subplot(1, 1, 1)
p1, = ax1.plot(dh2o_dp,pressure,'g',linewidth=3.0,label='Vertical Gradient of specific humidity')
if plot_logscale == True:
    ax1.set_yscale('log')
...
legend = ax1.legend(loc='upper right', shadow=True,fontsize=14)
ax1.set_ylabel('Pressure (hPa)',fontsize=label_fontsize)
ax1.set_xlabel('Vertical gradient specific humidity (g/kg/100 hPa)',fontsize=label_fontsize)
save_name = imagedir + "rrtm_vertmoisturegradient_profile.png"
plt.savefig(save_name, bbox_inches='tight')
