...
label_fontsize = 16
xlims = [0.,90.]
image_suffix = 'jetoption3_ubar50_jetlat45N'
...
delta = ubar/(beta*((wavelen*1000.)**2.0))
...
if 1 == 0:
    nticks = 6
    #yticks_ax1 = np.linspace(-10,10,nticks)
...
if 1 == 1:
    nticks = 6
    #yticks_ax1 = np.linspace(-10,10,nticks)
    #yticks_ax1 = np.logspace(-3,3,nticks)    
    yticks_ax1 = np.logspace(-3,2,nticks) 
    ...
    ax1.axvline(lat_info,linewidth=4, color='k')
    ax1.axhline(0.1,linewidth=4, color='k')
    p1, = ax1.plot(latitudes,np.abs(delta),'r',linewidth=3.0,label=r'$\delta = U/(\beta L^2)$')   
    legend = ax1.legend(loc='upper left', shadow=True)    
    ax1.grid(linestyle='-')
    ...
    ax1.set_ylabel(r'Nonlinearity, $\delta$ (dimensionless)',fontsize=label_fontsize)
    
    ax1.set_xlabel('Latitude (degrees)',fontsize=label_fontsize)
    save_name = imagedir + 'delta_nonlinearity_scale_RossbyWaves_' + image_suffix + '.png'
    plt.savefig(save_name, bbox_inches='tight')
    plt.show()
